...
import { FACULTY_ADVISER_KEY, RESEARCH_KEY } from '@/lib/constants';
...
export function RejectDialog({ id, disabled = false }: RejectDialogProps) {
  ...
  const update = useMutation({
    ...
    async onSuccess() {
      // prettier-ignore
      await queryClient.invalidateQueries({ queryKey: [RESEARCH_KEY, id] });
      await queryClient.invalidateQueries({ queryKey: [FACULTY_ADVISER_KEY] });
    },
  });
  ...
  async function rejectHandler() {
    try {
      ...
      toast({
        title: 'Reject Research Success',
      });

      setOpen(false);
    } catch (error) {
      toast({
        title: 'Reject Research Failed',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div>
          <p>Do you want to reject this research?</p>
        </div>
        <DialogFooter>
          <Button
            variant="destructive"
